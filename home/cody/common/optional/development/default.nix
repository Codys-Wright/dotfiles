...
let
  publicGitEmail = config.userSpec.email.gitHub;
  sshFolder = "${config.home.homeDirectory}/.ssh";
  publicKey =
    if config.userSpec.useYubikey then "${sshFolder}/id_yubikey.pub" else "${sshFolder}/id_manu.pub";
...
in
{
  ...
  programs.git = {
    userName = config.userSpec.handle;
  ...
  home.file."${privateGitConfig}".text = ''
    [user]
      name = "${config.userSpec.handle}"
      email = ${publicGitEmail}
  '';
  home.file."${workGitConfig}".text = ''
    [user]
      name = "${config.userSpec.userFullName}"
      email = "${workEmail}"
  '';
}
