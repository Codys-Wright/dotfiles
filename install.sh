...
nix-shell -p git --command "git clone https://gitlab.com/codys-wright/dotfiles $SCRIPT_DIR"
...
sed -i "0,/cody/s//$(whoami)/" $SCRIPT_DIR/flake.nix
sed -i "0,/Cody/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
sed -i "s/acodywright@gmail.com//" $SCRIPT_DIR/flake.nix
