...
let
  ...
  yubikeyHostsWithoutDomain =
    (lib.optional (
      config.hostSpec.networking.subnets ? grove
    ) config.hostSpec.networking.subnets.grove.wildcard)
    ++ (lib.optional (
      config.hostSpec.networking.subnets ? vm-lan
  ...
  genDomains = lib.map (h: "${h}.${config.userSpec.domain}");
  yubikeyHostAll =
  ...
  vanillaHostsConfig = lib.attrsets.mergeAttrsList (
    lib.lists.map (host: {
      "${host}" = lib.hm.dag.entryAfter [ "yubikey-hosts" ] {
        match = "host ${host},${host}.${config.userSpec.domain}";
        hostname = "${host}.${config.userSpec.domain}";
    ...
    in
    {
      ...
      matchBlocks =
        ...
        in
        {
          ...
          "gooey" = lib.mkIf (config.hostSpec.networking.subnets ? grove) (
            lib.hm.dag.entryAfter [ "yubikey-hosts" ] {
              host = "gooey";
              hostname = "gooey.${config.userSpec.domain}";
          ...
          "oops" = lib.mkIf (config.hostSpec.networking.ports.tcp ? oops) (
            lib.hm.dag.entryAfter [ "yubikey-hosts" ] {
              host = "oops";
              hostname = "oops.${config.userSpec.domain}";
              user = "${config.userSpec.username}";
