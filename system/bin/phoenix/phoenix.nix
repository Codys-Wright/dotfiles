...
let 
  phoenixScriptsDir = userSettings.dotfilesDir + "/system/bin/phoenix";
  myScript = ''
      if [ "$1" = "sync" ]; then
        if [ "$#" = 1 ]; then
          ''+phoenixScriptsDir+''/sync.sh;
          exit 0;
        elif [ "$2" = "user" ]; then
          ''+phoenixScriptsDir+''/sync-user.sh;
          exit 0;
        elif [ "$2" = "system" ]; then
          ''+phoenixScriptsDir+''/sync-system.sh;
          exit 0;
        else
