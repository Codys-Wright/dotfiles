...
}:
{
  ...
  programs.ssh = lib.optionalAttrs pkgs.stdenv.isLinux {
    ...
    knownHostsFiles =
      ...
      ++ lib.optional (!config.hostSpec.isMinimal && inputs.nix-secrets.networking.ssh ? knownHostsFileContents) (
        pkgs.writeText "custom_private_known_hosts" inputs.nix-secrets.networking.ssh.knownHostsFileContents
      )
      ++ lib.optional (config.hostSpec.isWork && inputs.nix-secrets ? work && inputs.nix-secrets.work ? ssh && inputs.nix-secrets.work.ssh ? knownHostsFileContents) (
